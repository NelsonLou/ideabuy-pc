var Validform = {
  ...
  datatype: {
    ...
    "*": /[\w\W]+/, //不能为空！
    "*8-16": /^[\w\W]{8,16}$/, //请填写8到16位任意字符！
    "n": /^\d+$/, //数字
    "n6-16": /^\d{6,16}$/, //请填写6到16位数字
    "s": /^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/, //不能输入特殊字符！
    "s6-18": /^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18}$/, //请填写6到18位字符！
    "p": /^[0-9]{6}$/, //邮政编码
    "m": /^13[0-9]{9}|14[0-9]{9}|15[0-9]{9}|17[0-9]{9}|18[0-9]{9}$/, //手机号
    "e": /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/, //邮箱
    ...
    "pwd": /^[a-zA-Z]\w{7,17}$/, // 8-16位密码
    "bank": /^(998801|998802|622525|622526|435744|435745|483536|528020|526855|622156|622155|356869|531659|622157|627066|627067|627068|627069)\d{10}$/ //银行卡号
  },
  formCheck: function(options) {
    ...
    var ipt = $(options.valiContent).find("input[datatype], select[datatype], textarea[datatype]");
    //改变验证类型
    ...
    //验证非空
    $(options.valiContent).on("blur", "[datatype]", function() {
